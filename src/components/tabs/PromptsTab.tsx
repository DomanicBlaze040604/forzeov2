      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3 overflow-x-auto pb-1 md:pb-0">
          <div className="flex items-center bg-gray-100 rounded-lg p-1">
            <button
              onClick={() => setPromptsTabView("active")}
              className={cn(
                "px-4 py-2 rounded-md text-sm font-medium transition-all",
                promptsTabView === "active"
            ...
            <button
              onClick={() => setPromptsTabView("suggested")}
              className={cn(
                "px-4 py-2 rounded-md text-sm font-medium transition-all",
            ...
            <button
              onClick={() => setPromptsTabView("inactive")}
              className={cn(
                "px-4 py-2 rounded-md text-sm font-medium transition-all",
            ...
            >
              <span className="flex items-center gap-2">
                ...
                <span className="px-1.5 py-0.5 rounded text-xs bg-gray-200 text-gray-500">
                  {inactiveCount}
                </span>
              </span>
            </button>
        ...
        <div className="flex items-center gap-3">
          <Select
            value={promptsFilterCompetitor}
            onValueChange={setPromptsFilterCompetitor}
          >
          ...
          <Select
            value={promptsFilterVisibility}
            onValueChange={(v: any) => setPromptsFilterVisibility(v)}
          >
            <SelectTrigger className="w-[130px] h-9 text-xs bg-white border-gray-200">
          ...
          <Button
            onClick={() => setBulkPromptsOpen(true)}
            size="sm"
            className="bg-gray-900 hover:bg-gray-800"
            disabled={!isAdmin && prompts.length >= (isAgency ? 15 : 200)}
          >
      ...
      <div className="flex items-center justify-between gap-3 bg-white p-3 rounded-xl border border-gray-200 shadow-sm">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              className="bg-white border-gray-200 hover:bg-gray-50 text-gray-700 font-medium shadow-sm transition-all duration-200"
            >
          ...
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuItem onClick={exportToCSV} className="cursor-pointer">
              <FileText className="h-4 w-4 mr-2 text-gray-400" /> Export Summary
              (CSV)
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={exportModelResponsesToCSV}
              className="cursor-pointer"
            >
              <FileText className="h-4 w-4 mr-2 text-blue-500" /> Export Raw
              Responses (CSV)
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <table className="w-full">
          <thead className="bg-gray-50/80 backdrop-blur-sm border-b border-gray-200 sticky top-0 z-10">
            <tr className="flex w-full">
              <th className="w-10 flex-none px-3 py-3 text-left" title="Select prompts for bulk actions">
                <Checkbox
                  checked={
              ...
              <th
                className="flex-1 min-w-0 px-2 py-3 text-left text-[10px] font-bold text-gray-400 uppercase tracking-wider"
                title="The question you want AI models to answer"
              >
              ...
              <th
                className="w-24 flex-none px-2 py-3 text-left text-[10px] font-bold text-gray-400 uppercase tracking-wider"
                title="Topic / seed keyword this prompt belongs to"
              >
                TOPIC
              </th>
              <th
                className="w-28 flex-none px-2 py-3 text-center text-[10px] font-bold text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-900 group"
                title="AI Opportunity reflects the relative strategic upside of improving visibility for this prompt in AI-generated answers."
                onClick={() => {
                  setPromptSortField((prev) =>
                    prev === "ai_opportunity" ? null : "ai_opportunity"
                  );
                }}
              >
              ...
              <th
                className="w-[70px] flex-none px-2 py-3 text-center text-[10px] font-bold text-gray-400 uppercase tracking-wider"
                title="How many AI models mentioned your brand vs total models tested"
              >
                VISIBILITY
              </th>
              <th
                className="w-[70px] flex-none px-2 py-3 text-center text-[10px] font-bold text-gray-400 uppercase tracking-wider"
                title="Your brand's average position in AI-generated ranked lists (#1 is best)"
              >
                POSITION
              </th>
              <th
                className="w-44 flex-none px-2 py-3 text-center text-[10px] font-bold text-gray-400 uppercase tracking-wider"
                title="Other brands that AI mentioned alongside or instead of yours"
              >
                BRANDS
              </th>
              <th
                className="w-[70px] flex-none px-2 py-3 text-center text-[10px] font-bold text-gray-400 uppercase tracking-wider"
                title="Number of web sources the AI referenced in its response"
              >
                CITATION
              </th>
              <th
                className="w-36 flex-none px-2 py-3 text-center text-[10px] font-bold text-gray-400 uppercase tracking-wider"
                title="AI-generated suggestions to improve your visibility"
              >
                RECOMMENDATIONS
              </th>
              {isAdmin && (
                <th
                  className="w-16 flex-none px-2 py-3 text-center text-[10px] font-bold text-gray-400 uppercase tracking-wider"
                  title="Run audits, edit, or archive prompts"
                >
                  ACTIONS
                </th>
              )}
            ...
            {rowVirtualizer.getVirtualItems().map((virtualRow) => {
              ...
              return (
                <tr
                  ...
                  className={cn("hover:bg-gray-50 transition-colors group border-b border-gray-50 last:border-0 absolute w-full flex items-start", isInactive && "opacity-60")}
                  style={{
                    top: 0,
                    transform: `translateY(${virtualRow.start}px)`,
                  }}
                >
                  <td className="w-10 flex-none px-3 py-4"><Checkbox checked={selectedPromptIds.has(p.id)} onCheckedChange={(checked) => { const newSet = new Set(selectedPromptIds); if (checked) { newSet.add(p.id); } else { newSet.delete(p.id); } setSelectedPromptIds(newSet); }} /></td>
                  <td className="flex-1 min-w-0 px-2 py-4">
                    <div className="flex flex-col gap-1.5 min-w-0">
                      <div className="flex items-start gap-2 min-w-0">
                        {isInactive && <Archive className="h-4 w-4 text-gray-400 flex-shrink-0 mt-0.5" />}
                        <span
                          onClick={(e) => {
                            e.stopPropagation();
                            if (isAdmin || r) {
                          ...
                          title={p.prompt_text}
                          className={cn(
                            "text-sm font-medium transition-colors break-words whitespace-normal",
                  ...
                  <td className="w-24 flex-none px-2 py-2.5">
                    {inlineEditTopicId === p.id ? (
                      ...
                      >
                        {p.topic ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-violet-50 text-violet-700 border border-violet-200 text-xs font-medium truncate max-w-[90px] group-hover/topic:border-violet-400 transition-colors">
                  ...
                  <td className="w-28 flex-none px-2 py-2.5 text-center">
                    {(() => {
                      ...
                      return (
                        <span
                          ...
                          style={{
                            backgroundColor: op.color + "18",
                            color: op.color,
                            border: `1px solid ${op.color}40`,
                          }}
                          title={
                            !op.demandDataAvailable
                              ? "Search volume data pending — score may change once fetched"
                              : `AI Opportunity score: ${op.score}`
                          }
                        >
                          {op.tier}
                          {!op.demandDataAvailable && (
                            <span
                              ...
                              title="Search volume data not yet available"
                            >
                              ~
                            </span>
                          )}
                  ...
                  <td className="w-[70px] flex-none px-2 py-2.5 text-center overflow-hidden">
                    <span
                      className={cn(
                        "text-sm font-medium",
                        visibleCount > 0 ? "text-gray-900" : "text-gray-400"
                      )}
                    >
                      {r ? `${visibleCount}/${totalCount}` : "\u2014"}
                    </span>
                  </td>
                  <td className="w-[70px] flex-none px-2 py-2.5 text-center overflow-hidden font-medium text-sm">
                    <span className={pos ? "text-blue-600" : "text-gray-400"}>
                      {pos
                        ? `#${typeof pos === "number" ? pos.toFixed(1) : pos}`
                        : "\u2014"}
                    </span>
                  </td>
                  <td className="w-44 flex-none px-2 py-2.5 overflow-hidden">
                    <div className="flex items-center justify-center gap-1">
                      {(() => {
                        ...
                        const displayBrands = brandsArray.slice(0, 4);
                            ...
                            {displayBrands.map((brand, idx) => {
                              ...
                              return (
                                <div
                                  ...
                                  title={brand}
                                  className={cn(
                                    "flex-none flex items-center justify-center h-5 w-5 rounded-md border transition-all hover:scale-110",
                                    isUserBrand
                  ...
                  <td className="w-[70px] flex-none px-2 py-4 text-center">
                    {r ? (
                      <div className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-50 text-blue-700 text-xs font-bold border border-blue-100 shadow-sm">
                        {cit}
                      </div>
                    ) : (
                  ...
                  <td className="w-36 flex-none px-2 py-2.5 text-center">
                    {r ? (
                      <Button
                        variant="outline"
                        size="sm"
                  ...
                  {isAdmin && (
                    <td className="w-16 flex-none px-2 py-2.5">
                      <div className="flex items-center justify-center gap-1">
                        {isInactive ? (
                          <>
              ...
              <div className="flex items-center gap-4 min-w-[300px]">
                ...
                <div className="flex-1 h-2 bg-gray-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(bulkRunProgress!.current / bulkRunProgress!.total) * 100}%` }}
                  ></div>
                </div>
                <span className="text-sm font-medium whitespace-nowrap">
                  {bulkRunProgress!.current} / {bulkRunProgress!.total}
                </span>
              </div>
            ) : (
              <>
